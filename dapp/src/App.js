...
function getContractAddress() {
  return "0xE8f3BEF9e6f79e72Dd491A10B1982B069F0DfcBd";
}
...
function App() {
  const [gabrielToken, setGabrielToken] = useState(0);
  ...
  useEffect(() => {
    ...
    if (loaded && (accounts !== 0)) {
      ...
      gabrielToken.events.Staked(options)
      // data is when 
  ...
  async function getUserProfile() {
    ...
    call(gabrielToken.methods.totalSupply, setTotalSupply);
    // balanceOf Requires input argument of the account to grab, so let's grab the first available account for now
    call(gabrielToken.methods.balanceOf, setAccountBalance, accounts[0]);
    call(gabrielToken.methods.hasStake, setAccountStakes, accounts[0]);
  ...
  function stake() {
    ...
    gabrielToken.methods.stake(1000).estimateGas({from: accounts[0]})
      .then((gas) => {
        // We now have the gas amount, we can now send the transaction
        gabrielToken.methods.stake(1000).send({
          from: accounts[0],
          gas: gas
        })
        .then(() => {
